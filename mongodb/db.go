...
func (l *Logger) RecordRequest(id string, req *http.Request) error {
	...
	if originIP == "" {
		log.Info().
			Str("context-id", id).
			Msgf("No origin IP found for request.")
		return nil
	}
	...
	if mgm.Coll(&m).First(bson.M{"ip_address": originIP}, &m, findOptions) != nil {
		log.Info().
			Str("context-id", id).
			Str("origin-ip", originIP).
			Msgf("No mapping found for origin IP, skipping request.")
		return nil
	}
	...
	var class = "app"
	if m.Platform == "android" && strings.HasPrefix(req.UserAgent(), "com.android.vending") {
		class = "com.android.vending"
	} else {
		var backgroundRequest = request{}
		if mgm.Coll(&backgroundRequest).First(bson.M{
			"package": "background",
			"platform": m.Platform,
			"host": req.Host,
	...
	if mgm.Coll(&request).Create(&request) != nil {
		log.Error().
			Str("context-id", id).
			Str("origin-ip", originIP).
			Str("package", m.Package).
